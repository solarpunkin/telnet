...
static const unsigned char *find_key(const unsigned char *buf, size_t len, const char *key, size_t *val_start, size_t *val_len) {
    ...
    const char *needle_fmt = "%zu:%s"; // not used
    // brute force scan for key string
    size_t keylen = strlen(key);
    for (const unsigned char *p = buf; p < end; ++p) {
        // look for patter "<keylen>:<key>"
        // first find ":" the n backtrack digits
        if( *p >= '0' && *p <= '9') {
            ...
            if (s + sl > end) { p = col; continue; }
            if (sl == (long)keylen && memcmp(s, key, keylen) == 0) {
                // value starts after this key string: move to after key
                const unsigned char *after_key = s + sl;
                // now parse the next bencoded value
                size_t vlen = 0;
                const unsigned char *valptr = scan_bencoded_value(after_key, end, &vlen);
...
int storage_init_from_info(storage_t *storage, const unsigned char *info_buf, size_t info_len, const char *torrent_name) {
    ...
    if (!storage->pieces_hashes) return -1;
...
/* verify piece hash (reads entire piece and SHA256 it) */
static int verify_piece_sha256(storage_t *s, uint32_t index) {
    ...
    if (s->piece_hash_len == 32) {
        ...
        // compute SHA1 of buf
        #include <openssl/sha.h>
        SHA_CTX ctx1; SHA1_Init(&ctx1); SHA1_Update(&ctx1, buf, psize); SHA1_Final(sha1d, &ctx1);
